...
async function getRefundPolicy() {
  const query = `*[_type == "refundPolicy"][0]`; // Fetch the first privacy policy document
  const refundPolicy = await sanityClient.fetch(query);
  return refundPolicy;
}
...
export default async function RefundPolicyPage() {
  ...
  return (
    <div className='mt-[16rem] mb-[3rem]'>
      <h1 className='text-[2.5rem] text-center'>{refundPolicy.title}</h1>
      <div className='px-[5.5rem] mt-[5rem]'>
       <PortableText value={refundPolicy.content} components={components} />
      </div>
    </div>
  );
}
